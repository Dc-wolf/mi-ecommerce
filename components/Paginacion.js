...
export default function Paginacion({ paginaActual, totalPaginas, buscar }) {
  ...
  return (
    <div className="flex flex-wrap justify-center items-center gap-2 mt-10 mb-10 px-4">

      {/* Anterior */}
      {paginaActual > 1 ? (
        <Link
          href={`/?pagina=${paginaActual - 1}${q}`}
          className="px-2 sm:px-3 py-2 bg-black text-white rounded-lg hover:bg-gray-800 text-sm font-medium"
        >
          ← <span className="hidden sm:inline">Anterior</span>
        </Link>
      ) : (
        <span className="px-2 sm:px-3 py-2 bg-gray-200 text-gray-400 rounded-lg text-sm">
          ←
        </span>
      )}
      ...
      {paginasVisibles.map((num, index) => {
        ...
        return (
          <span key={num} className="flex items-center gap-1">
            ...
            <Link
              href={`/?pagina=${num}${q}`}
              className={`w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-lg border text-xs sm:text-sm font-medium transition
              ${
                num === paginaActual
